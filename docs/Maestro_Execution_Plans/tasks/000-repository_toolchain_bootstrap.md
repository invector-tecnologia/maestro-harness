...
* **Context Anchors:** #file:docs/Maestro_Manifesto/ARCHITECTURE.md, #file:docs/adr/0001-rust-core-nim-niobium-tui-stdio-protocol.md, #file:.github/instructions/rust-companion.instructions.md
* **Expected Output:** A compiling Rust `maestro` crate and a Nim/Niobium `frontend/` skeleton, both green in CI.
...
* The Nim frontend depends on `niobium >= 0.1.0`; it never re-implements TUI primitives.
...
4. Scaffold `frontend/` (`maestro_tui.nimble` requiring `niobium >= 0.1.0`, `src/app.nim`, `src/protocol.nim`, `tests/`).
...
* **AC3:** `frontend/maestro_tui.nimble` requires `niobium >= 0.1.0` and the project compiles/tests headlessly.
...
* **AC3:** `niobium 0.1.1` resolved via `nimble`; `nimble build` produces `maestro_tui`; `nimble test` → 3 passed (headless protocol framing, no TTY).
