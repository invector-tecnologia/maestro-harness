# TASK 052: Nim/Tatui TUI Shell

## 1. TASK SIGNATURE (DSPy Architecture)
* **Inputs:** IPC stdio protocol, Tatui dependency.
* **Context Anchors:** #file:.github/skills/tatui-frontend/SKILL.md, #file:.github/instructions/nim-frontend.instructions.md
* **Expected Output:** A `frontend/` Nim process rendering Maestro state via Tatui.

## 2. ABSOLUTE CONSTRAINTS (1.58-bit Constraint)
* Consume Tatui only (`requires "tatui >= 0.1.0"`); never re-implement TUI primitives.
...
Goal: Build the Nim/Tatui TUI shell: tick loop, protocol client, and the base panel layout.
...
Execute:
1. Scaffold `frontend/maestro_tui.nimble` requiring `tatui >= 0.1.0`.
...
4. Add golden-snapshot tests using Tatui's test backend (no TTY).
...
* **AC1:** The TUI builds against `tatui >= 0.1.0` and runs headless in tests.
